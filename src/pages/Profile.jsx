...
class Profile extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.Logout.token !== this.props.Logout.token) {
      return this.props.navigate("/login");
    }
    const userinfo = JSON.parse(localStorage.getItem("userInfo")) || null;
    if (prevProps.Profile.data !== this.props.Profile.data) {
      return this.props.dispatch(doProfileAction(userinfo.id, userinfo));
    }
    if (!userinfo) return this.props.navigate("/login");
  ...
  changePassword = () => {
    ...
    if (this.state.oldPwd && this.state.newPwd && this.state.ConPwd) {
      if (this.state.newPwd !== this.state.ConPwd)
        return this.setState({ notsameErr: "Passwords are not the same" });
      const data = {
  ...
  sendQuery = () => {
    ...
    if (!this.state.newPicture && this.state.deletepictrue)
      imageFile = this.state.deletepictrue;
    if (imageFile) {
      bodyFormDataProfile.append("image", imageFile);
    }
    if (this.state.newDisplayname)
      bodyFormDataProfile.append("display_name", this.state.newDisplayname);
    if (this.state.newFirstName)
      bodyFormDataProfile.append("firstname", this.state.newFirstName);
    if (this.state.newLastName)
      bodyFormDataProfile.append("lastname", this.state.newLastName);
    if (this.state.newAddress)
      bodyFormDataProfile.append("adress", this.state.newAddress);
    if (this.state.newGender)
      bodyFormDataProfile.append("gender", this.state.newGender);
    if (this.state.newBorn)
      bodyFormDataProfile.append("date_of_birth", this.state.newBorn);
    if (this.state.newEmail && this.state.newPhone)
      bodyFormDataUser = {
        email: this.state.newEmail,
        phone: this.state.newPhone,
      };
    if (this.state.newEmail && !this.state.newPhone)
      bodyFormDataUser = { email: this.state.newEmail };
    if (this.state.newPhone && !this.state.newEmail)
      bodyFormDataUser = { phone: this.state.newPhone };
    let countPorfile = 0;
    let countUser = 0;
    ...
    if (countPorfile > 0)
      this.props.dispatch(
        doUpdateProfileAction(bodyFormDataProfile, userinfo.token)
      );
    if (bodyFormDataUser)
      this.props.dispatch(doUpdateUserAction(bodyFormDataUser, userinfo.token));
  };
  ...
  selectImage = () => {
    if (
      !this.props.Profile.dataProfile[0].displaypicture &&
      !this.state.newPicture &&
      !this.state.deletepictrue
    )
      return this.state.pictrue;
    if (this.state.newPicture && !this.state.pictrue)
      return URL.createObjectURL(this.state.newPicture);
    if (!this.state.newPicture && this.state.deletepictrue)
      return this.state.deletepictrue;
        ...
        {!this.props.Profile.isLoading &&
          this.props.Profile.dataProfile.length > 0 && (
            ...
            <NavbarMobile/>
              ...
              {!this.state.editPwd ? undefined : (
                <div className={styles.modalPwd}>
                  <div className={styles["modal-top"]}>
                    <i
                      className={`fa-regular fa-rectangle-xmark ${styles.iconPwd}`}
                      onClick={() => {
                        this.setState({
                          editPwd: false,
                        });
                  ...
                  <div>
                    <form className={styles["modal-middle"]}>
                      ...
                      <input
                        type={this.state.shwPwd2 ? "text" : "password"}
                        onChange={(e) => {
                          this.setState({
                      ...
                      ></input>
                      <i
                        className={`${this.iconShow2()} ${styles.passwords2}`}
                        onClick={() => {
                          this.setState((prevState) => ({
                      ...
                      <input
                        type={this.state.shwPwd3 ? "text" : "password"}
                        onChange={(e) => {
                          this.setState({
                      ...
                      ></input>
                      <i
                        className={`${this.iconShow3()} ${styles.passwords3}`}
                        onClick={() => {
                          this.setState((prevState) => ({
                  ...
                  {!this.props.Profile.isError && (
                    <p className={styles["sukses"]}>
                      {this.props.Profile.resPwd}
                    </p>
                  )}
                  {this.props.Profile.isError && (
                    <p className={styles["error"]}>
                      {this.props.Profile.errPwd}
                    </p>
                  )}
                  {this.state.notsameErr && (
                    <p className={styles["error"]}>{this.state.notsameErr}</p>
                  )}
                  <div
              ...
              <main
                className={`${styles.bg} ${
                  this.state.editPwd && styles["show-modal"]
                }`}
              >
                ...
                <section className={styles["profile-section"]}>
                  <aside className={styles["content-img"]}>
                    ...
                    <div className={styles["user-content"]}>
                      ...
                      <p className={styles["content-email"]}>
                        {this.props.Profile.dataUser[0].email}
                      </p>
                    </div>
                    ...
                    {this.state.isEdit && (
                      <div className={styles["set-display"]}>
                        <p className={styles["content-change"]}>
                    ...
                    <button
                      className={`${styles.logout} ${styles["width-pc"]}`}
                      onClick={() => {
                          ...
                          <div className={styles.adress}>
                            ...
                            <input
                              ...
                              placeholder={
                                this.state.isEdit
                                  ? "Input Address..."
                                  : this.props.Profile.dataProfile[0].adress
                              }
                              id="adress"
                              onChange={(e) => {
                                this.setState({
                        ...
                        <aside className={styles["right-input"]}>
                          ...
                          <input
                            ...
                            placeholder={
                              this.state.isEdit
                                ? "Input Phone Number..."
                                : this.props.Profile.dataUser[0].phone
                            }
                            id="phone"
                            onChange={(e) => {
                              this.setState({
                    ...
                    <div>
                      ...
                      <form className={styles.profile}>
                          ...
                          <div className={styles["display-name"]}>
                            ...
                            <input
                              ...
                              placeholder={
                                this.state.isEdit
                                  ? "Input Display Name..."
                                  : this.props.Profile.dataProfile[0]
                                      .display_name
                              }
                              id="displayname"
                              onChange={(e) => {
                          ...
                          <div className={styles["first-name"]}>
                            ...
                            <input
                              ...
                              placeholder={
                                this.state.isEdit
                                  ? "Input First Name..."
                                  : this.props.Profile.dataProfile[0].firstname
                              }
                              id="firstname"
                              onChange={(e) => {
                                this.setState({
                          ...
                          <div className={styles["last-nam"]}>
                            ...
                            <input
                              ...
                              placeholder={
                                this.state.isEdit
                                  ? "Input Last Name..."
                                  : this.props.Profile.dataProfile[0].lastname
                              }
                              id="lastname"
                              onChange={(e) => {
                                this.setState({
                        ...
                        <div className={styles["right-input"]}>
                          ...
                          <input
                            type={this.state.isEdit ? "date" : "text"}
                            defaultValue={
                              this.state.isEdit ? "" : this.getBorn()
                            }
                            disabled={this.state.isEdit ? false : true}
                            placeholder={
                              this.state.isEdit
                        ...
                        <form className={styles["gender-form"]}>
                          <div className={styles.male}>
                            <input
                              ...
                              defaultChecked={
                                !this.state.isEdit
                                  ? this.props.Profile.dataProfile[0].gender ===
                                    "male"
                                    ? true
                                    : false
                                  : false
