...
class Product extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      this.props.dispatch(
        productsActions.getProductSelectAction(
  ...
  render() {
    ...
    return (
      <>
        <div className={styles["navbar-pc"]}>{this.state.navbar}</div>
        <main className={styles["product-main"]}>
          <aside className={styles["left-content"]}>
            ...
            <div className={styles["promo-ipad"]}>
              ...
              {!this.props.promo.isLoading ? (
                this.props.promo.data.map((promo, index) => {
                  if (promo.discount !== 0 && promo.title !== "testing")
                    return (
                      <CardPromo
                        key={index}
                        id={promo.id}
            ...
            <p className={`${styles.btn} ${styles["Apply-btn"]}`}>
              Apply Coupon
            </p>
            <br />
                ...
                <p
                  onClick={() => {
                    this.setState((prevState) => ({
                      isEditPromo: prevState.isEditPromo ? false : true,
                    }));
                  }}
                >
                  Edit promo
                </p>
                <Link className={styles.link} to={"/promo/new"}>
                  <p>Add new promo</p>
                </Link>
          ...
          <aside className={styles["right-content"]}>
            <ul>
              <li
                className={styles[this.state.categoryPromo]}
                onClick={() => {
                  this.props.dispatch(
                    productsActions.getProductAction("page=1&limit=12")
                  );
                  this.setState({
                    categoryPromo: "start-content",
                    categoryFoods: "next-content",
              ...
              <li
                className={styles[this.state.categoryNonCoffee]}
                onClick={() => {
              ...
              <li
                className={styles[this.state.categoryAddOn]}
                onClick={() => {
            ...
            <section
              className={`container-fluid text-center ${styles.cardPadding}`}
            >
              <div
                    ...
                    if (product.id !== 999)
                      return (
                        <Card
                          ...
                          discount={this.checkDiscount(product.discount)}
                          image={product.image}
                          id={product.id}
            ...
            <div className={styles.bungkusan}>
              <p
                className={this.props.products.tglprev}
                onClick={() => {
                  if (this.props.products.tglprev !== styles.hide)
                    return this.getData(
                      !this.props.products.isLoading && this.props.products.prev
                    );
                }}
              >
                PREV
              ...
              <p
                className={this.props.products.tglnext}
                onClick={() => {
                  if (this.props.products.tglnext !== styles.hide)
                    return this.getData(
                      !this.props.products.isLoading && this.props.products.next
                    );
                }}
              >
                NEXT
            ...
            {admin && (
              <div className={styles.admin}>
                <p
                  onClick={() => {
                    this.setState((prevState) => ({
                      isEdit: prevState.isEdit ? false : true,
                    }));
                  }}
                >
                  Edit product
