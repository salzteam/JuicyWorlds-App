...
sendAll = () => {
  const {password, conpassword, code } = this.state
  if (!password) return this.setState({errMsg: "Input Passoword!"})
  if (!conpassword) return this.setState({errMsg: "Input Confirm Passoword!"})
  if (password !== conpassword) return this.setState({errMsg: "Password doesn't match!"})
  const url = `${process.env.REACT_APP_BACKEND_HOST}/api/v1/auth/`
  this.setState({isLoading:true, errMsg: null})
  Axios.patch(url, {code:code, new_password:password}).then(()=>{
    this.setState({isLoading:false},()=>{this.props.navigate("/login")})
  })
}
...
checkCode = () => {
  ...
  if (code === pin) {
    return this.setState({isPswd:true, isCode: false, isFooter: true, errMsg: null})
  } 
...
sendEmail = () => {
  ...
  if (this.state.email) { 
    ...
    this.setState({isLoading: true})
    Axios.patch(url, {email:data}).then((response)=>{
      // console.log(response);
      this.setState({pin: response.data.data.code, isLoading:false},()=>{!this.state.isLoading && this.setState({isEmail:false, isCode: true, isFooter: false, errMsg: null, minutes: 2},()=>{this.countDown()});})
    }).catch((err) => {
      if (err.response.data.message === "Code already send to email!") return this.setState({errMsg: "*** Code Already Send ! If you not in page input code please wait 2 more minutes again! ***", isLoading:false})
      if (err.response.data.message === "Email/Password is Wrong") return this.setState({errMsg: "*** Email/Password is Wrong! ***", isLoading:false})
    })  
...
iconShoww = () => {
  ...
  render(){
    const { minutes, seconds, isCode, isPswd,isEmail, isFooter, isLoading, errMsg } = this.state
    return <>
        ...
        <div className="row">
          ...
          <aside className={`col-6 ${styles.right}`}>
            <section>
              ...
              <div className={styles["input-btn"]}>
                <input type="email" className={!isEmail ? styles.hide : undefined} placeholder={"Enter your email adress to get link"} onChange={(e)=>{
                  this.setState({
                    email: e.target.value
                  })
                }}/>
                <input type="text" className={!isCode ? styles.hide : undefined} placeholder={"Input Code Here"} onChange={(e)=>{
                  this.setState({
                    code: e.target.value
                  })
                }}/>
                  <input type={this.state.shwPwd?"text":"password"} className={!isPswd ? styles.hide : undefined} placeholder={"New Password"} onChange={(e)=>{
                    this.setState({
                      password: e.target.value
                    })
                  }}/>
                  <i className={isPswd ? `${this.iconShoww()} ${styles.passwords}`: styles.hide} onClick={() => {
                this.setState((prevState) => ({
                  shwPwd: prevState.shwPwd ? false : true,
                }));
            }}></i>
                  <input type={this.state.shwPwdd?"text":"password"} className={!isPswd ? styles.hide : undefined} placeholder={"Confirm Password"} onChange={(e)=>{
                    this.setState({
                      conpassword: e.target.value
                    })
                  }}/>
                  <i className={isPswd ? `${this.iconShoww()} ${styles.passwordss}`: styles.hide} onClick={() => {
                this.setState((prevState) => ({
          ...
          <div className={styles.spinner}></div>
              ...
              <div className={styles["resend-btn"]}>
                ...
                <div className={minutes === 0 && seconds === 0 ? `${styles.btn} ${styles.resend}` : `${styles["btn-none"]} ${styles.resend}`}>
                  <p onClick={()=>{minutes === 0 && seconds === 0 && this.sendEmail()}}>Resend Link</p>
                </div>
              </div>}
            </section>
            <footer className={!isFooter ? styles["footer-container"] : styles["footer-padding"]}>
