...
class Register extends React.Component {
  ...
  errorModal = () => {
    this.setState({
      showModal: (
        <Modal title={"Register Error"} body={"Invalid Input Data"} tos={"/"} />
      ),
    });
  };
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.Register.isError !== this.props.Register.isError) {
      this.setState({
        errMsg: this.props.Register.err,
      });
    }
  ...
  toRegister = () => {
    ...
    if (email && !password && !phone)
      return this.setState({ errMsg: "Please Input Password and Phone !" });
    if (!email && password && !phone)
      return this.setState({ errMsg: "Please Input Email and Phone !" });
    if (!email && !password && phone)
      return this.setState({ errMsg: "Please Input Email and Password !" });
    if (email && password && !phone)
      return this.setState({ errMsg: "Please Input Phone !" });
    if (email && !password && phone)
          ...
          <section className={styles["section-register"]}>
            ...
            <form className={styles["register-form"]}>
              ...
              <input
                className={styles["register-input"]}
                type="tel"
                placeholder="Enter your phone number (08)"
                onChange={(e) => {
                  this.setState({
